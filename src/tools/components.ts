  ...
  register(server: McpServer, config: AuthConfig) {
    server.registerTool(
      'list_file_components',
      {
        ...
        inputSchema: inputSchemas.list_file_components,
      },
      async ({ file_key }) => {
        try {
          const result = await listFileComponents(config, { file_key });
          if (result.count === 0) return toolResult('No published components in this file. Components must be published to appear.', result);
  ...
  register(server: McpServer, config: AuthConfig) {
    server.registerTool(
      'list_file_styles',
      {
        ...
        inputSchema: inputSchemas.list_file_styles,
      },
      async ({ file_key }) => {
        try {
          const result = await listFileStyles(config, { file_key });
          if (result.count === 0) return toolResult('No published styles in this file.', result);
  ...
  register(server: McpServer, config: AuthConfig) {
    server.registerTool(
      'list_team_components',
      {
        ...
        inputSchema: inputSchemas.list_team_components,
      },
  ...
  register(server: McpServer, config: AuthConfig) {
    server.registerTool(
      'list_team_styles',
      {
        ...
        inputSchema: inputSchemas.list_team_styles,
      },
      async ({ team_id, page_size, cursor }) => {
        try {
